...
module.exports = async (launchpadsStateChangedData) => {
	...
	const savedLaunchpadAddresses = await LaunchpadService.getUpdatedStateLaunchpadsFromInputOne({
		launchpadAddresses: eventLaunchpadAddresses,
	})
	const newLaunchpadsStateChanged = launchpadsStateChangedData.filter((launchpadParameter) =>
		savedLaunchpadAddresses.includes(launchpadParameter.launchpad)
	)
