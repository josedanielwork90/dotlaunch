...
import walletManager, { DemoWalletProvider } from '../../../services/wallet';
...
const ToolBar = () => {
  ...
  const demoAvailable = DemoWalletProvider.isAvailable();
  ...
  // Restore a previous session once on mount, then follow the wallet
  // manager. The original effect had no dependency array, so it re-ran on
  // every render and re-queried the wallet continuously.
  useEffect(() => {
    handleGetConnectedMetaMask()
    return walletManager.subscribe(setWalletAddr)
  }, []);
  ...
  return (
    <ToolBarContainer>
      <div className="flex items-center w-1/2 space-x-3">
        ...
        <h3 className="hidden sm:block font-black text-xl">DotLaunch</h3>
      </div>
      <div className="flex space-x-4 w-1/2 items-center justify-end">
        ...
        <div className="hidden md:block">
          <img src={BinanceLogo} alt="" width={25} style={{ display: "inline-block", marginRight: "5px" }} />
          <span>{runtimeConfig.chainName}</span>
        </div>
        {/*<div className="hidden md:block">
        ...
        >
          {walletAddr ? (
            <div>
              {demoAvailable ? (
                <MenuItem>
                  <div className="flex space-x-3 items-center py-2" onClick={() => setDemoModalOpen(true)}>
                    <i className="las la-exchange-alt text-2xl" style={{ width: 30 }}></i>
              ...
              <MenuItem>
                <div className="flex space-x-3 items-center py-2" onClick={handleDisconnectMetaMask}>
                  <img src={MetamaskLogo} width="30" alt="" /><span>Disconnect</span>
                </div>
              </MenuItem>
            </div>
          ) : (
            <div>
              {demoAvailable ? (
                <MenuItem>
                  <div className="flex space-x-3 items-center py-2" onClick={() => setDemoModalOpen(true)}>
                    <i className="las la-user-astronaut text-2xl" style={{ width: 30 }}></i>
              ...
              <MenuItem>
                <div className="flex space-x-3 items-center py-2" onClick={handleConnectMetaMask}>
                  <img src={MetamaskLogo} width="30" alt="" /> <span>Metamask</span>
                </div>
              </MenuItem>
              <MenuItem>
                <div className="flex space-x-3 items-center py-2" onClick={connectWalletConnect}>
                  <img src={CoinbaseLogo} width="30" alt="" />
