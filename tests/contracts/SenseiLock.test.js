...
describe("SenseiLock", () => {
  ...
  describe("unlocking", () => {
    it("refuses before the unlock date", async () => {
      const { id } = await lockFor({ afterSeconds: 10 * DAY });

      await expect(lock.connect(alice).unlock(id)).to.be.revertedWith(
        "It is not time to unlock"
      );
    });

    it("refuses one second before the unlock date", async () => {
      const { id, unlockDate } = await lockFor();
      await time.increaseTo(unlockDate - 2);
    ...
    it("releases exactly at the unlock date", async () => {
      const { id, unlockDate } = await lockFor();
      await time.increaseTo(unlockDate);

      await expect(lock.connect(alice).unlock(id)).to.not.be.reverted;
    });

    it("returns the tokens to the lock's owner", async () => {
    ...
    it("refuses a withdrawal by anyone but the owner", async () => {
      const { id, unlockDate } = await lockFor({ holder: alice });
      await time.increaseTo(unlockDate);

      await expect(lock.connect(bob).unlock(id)).to.be.revertedWith(
        "You are not the owner of this lock"
      );
    });

    it("refuses a withdrawal by the address that paid for the lock", async () => {
      const { id, unlockDate } = await lockFor({ holder: alice, from: owner });
      await time.increaseTo(unlockDate);

      await expect(lock.connect(owner).unlock(id)).to.be.revertedWith(
        "You are not the owner of this lock"
      );
    });
    ...
    it("zeroes the lock rather than deleting it, so ids stay stable", async () => {
      const { id, unlockDate } = await lockFor();
      await time.increaseTo(unlockDate);
      await lock.connect(alice).unlock(id);
      ...
      expect(await lock.getTotalLockCount()).to.equal(1);
    });

    it("removes the token from the cumulative table once fully released", async () => {
      const { id, unlockDate } = await lockFor();
      await time.increaseTo(unlockDate);
      await lock.connect(alice).unlock(id);
      ...
      expect(await lock.allNormalTokenLockedCount()).to.equal(0);
    });

    it("keeps the token listed while another lock on it remains", async () => {
      const first = await lockFor({ holder: alice, afterSeconds: DAY });
      await lockFor({ holder: bob, afterSeconds: 60 * DAY });

      await time.increaseTo(first.unlockDate);
      await lock.connect(alice).unlock(first.id);
    ...
    it("drops the lock from the user's list", async () => {
      const { id, unlockDate } = await lockFor({ holder: alice });
      await time.increaseTo(unlockDate);
      await lock.connect(alice).unlock(id);

      expect(await lock.normalLockCountForUser(alice.address)).to.equal(0);
    });

    it("emits LockRemoved", async () => {
      const { id, unlockDate } = await lockFor();
      await time.increaseTo(unlockDate);

      await expect(lock.connect(alice).unlock(id)).to.emit(lock, "LockRemoved");
    });

    it("rejects an unknown lock id", async () => {
      await expect(lock.connect(alice).unlock(99)).to.be.revertedWith(
        "Invalid lock id"
      );
    });
  });
